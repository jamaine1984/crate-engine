...
class CharacterController {
  constructor(scene, camera, objects) {
    ...
    this.collider = collisionWorld.createPlayerCollider(2.8, 0.5);
  }
  ...
  getWeaponData() {
      ...
      if (!this.isGrounded) {
        ...
        if (this.position.y <= groundY || this.position.y < -1) {
          ...
          if (this._wasFalling && this._fallStartY !== undefined) {
            ...
            if (fallDist > 3) this._createSprintDust();
          }
    ...
    if (hasInput && !this.isRolling && !this.isAttacking) {
      ...
      this.speed = THREE.MathUtils.lerp(this.speed, targetSpeed, dt * 14);
    ...
    // === SWIMMING SYSTEM ===
    const WATER_LEVEL = -2.0; // Well below ground — only triggers in actual water
