...
async function main() {
  ...
  if (adminToken) {
    ...
    if (authed.status !== 200 || !authed.ok || !authed.dryRun || authed.deleted !== 0 || authed.errors !== 0) {
  ...
  if (authed) {
    console.log(`Authed dry run: scanned ${authed.scanned}, orphaned ${authed.orphaned}, deleted ${authed.deleted}, errors ${authed.errors}`);
  } else {
